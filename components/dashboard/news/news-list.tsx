...
export function NewsList({ items }: NewsListProps) {
  ...
  const [selectedItem, setSelectedItem] = useState<Doc<"news"> | null>(null);

  const handleEditClick = (item: Doc<"news">) => {
    setSelectedItem(item);
    setIsEditDialogOpen(true);
  };

  const handleDeleteClick = (item: Doc<"news">) => {
    setSelectedItem(item);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteNews({ id: selectedItem._id });
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-16">
        ...
        <p className="text-muted-foreground">
          There are no news articles to display for this filter.
        </p>
      </div>
    );
  }
